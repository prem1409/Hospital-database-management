...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	if(empty($myroomno) ||empty($myfname) || empty($mylname) || empty($mysex)|| empty($myphnumber)|| empty($myaddress)|| empty($myage) )
		echo "<font color = 'red' size='4'><center><u> You did not fill out the required fields.</u></center></font>";
	else
	{
	
	$query = mysqli_query($db, "INSERT INTO admit (roomno,fname,lname,sex,phnumber,age,address)VALUES ('$myroomno','$myfname','$mylname','$mysex','$myphnumber','$myage','$myaddress' )");

	
	if($query)
	{
		$last_id = mysqli_insert_id($db);
		echo "<font size='6' color = white><center> Thank You! you are now registered as an Admittied Patient with ID $last_id.!!</center></font>";
	}
	}
...
<center><font size = "8"><b>ADMIT LOGIN</b></font><br></center>
<br>
<form action="admit.php" method = "post">               

<font size = "5">Room Number:</font><br>
